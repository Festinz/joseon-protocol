...
export function buildEnvironment() {
  ...
  new THREE.TextureLoader().load('assets/real_hanok_row_cut.png?v=1', (tex) => {
    tex.colorSpace = THREE.SRGBColorSpace;
    const W = 11, H = W / (tex.image.width / tex.image.height);
    const hmat = new THREE.MeshBasicMaterial({ map: tex, transparent: true, alphaTest: 0.06, color: 0x9aa4c8, depthWrite: true, side: THREE.DoubleSide });
    const hgeo = new THREE.PlaneGeometry(W, H);
    for (const [x, z, flip] of [[-20.5, -6, 1], [-20.5, -19, -1], [-20.5, -33, 1], [20.5, -10, -1], [20.5, -24, 1], [20.5, -38, -1]]) {
      const p = new THREE.Mesh(hgeo, hmat);
      p.position.set(x, H / 2 - 0.3, z);
  ...
  new THREE.TextureLoader().load('assets/real_hanok_far_cut.png?v=1', (tex) => {
    ...
    const fmat = new THREE.MeshBasicMaterial({ map: tex, transparent: true, alphaTest: 0.06, color: 0x707ca4, depthWrite: true });
    const p1 = new THREE.Mesh(new THREE.PlaneGeometry(W, H), fmat);
    p1.position.set(-17, H / 2 + 1.2, -48); env.add(p1);
    const p2 = p1.clone(); p2.position.set(18, H / 2 + 0.9, -47.5); p2.scale.x = -1; env.add(p2);
  });
